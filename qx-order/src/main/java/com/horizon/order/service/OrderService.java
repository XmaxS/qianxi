package com.horizon.order.service;
...
import com.horizon.common.utils.IdWorker;
import com.horizon.common.utils.JsonUtils;
import com.horizon.common.vo.PageResult;
import com.horizon.item.pojo.Sku;
import com.horizon.order.client.AddressClient;
import com.horizon.order.client.GoodsClient;
import com.horizon.order.dto.AddressDTO;
import com.horizon.order.dto.OrderDto;
import com.horizon.order.dto.OrderStatusEnum;
import com.horizon.order.filter.LoginInterceptor;
import com.horizon.order.mapper.OrderDetailMapper;
import com.horizon.order.mapper.OrderMapper;
import com.horizon.order.mapper.OrderStatusMapper;
import com.horizon.order.pojo.Order;
import com.horizon.order.pojo.OrderDetail;
import com.horizon.order.pojo.OrderStatus;
import com.horizon.order.utils.PayHelper;
...
public class OrderService {
    ...
    public Long createOrder(OrderDto orderDto) {
        ...
        if (addressDTO == null) {
            // 商品不存在，抛出异常
            throw new QxException(ExceptionEnums.RECEIVER_ADDRESS_NOT_FOUND);
        }
    ...
    public String generateUrl(Long orderId) {
        ...
        if (order.getOrderStatus().getStatus() != OrderStatusEnum.INIT.value()) {
            throw new QxException(ExceptionEnums.ORDER_STATUS_EXCEPTION);
        }

        //todo 这里传入一份钱，用于测试使用，实际中使用订单中的实付金额
        String url = payHelper.createPayUrl(orderId, "乐优商城测试", /*order.getActualPay()*/1L);
        if (StringUtils.isBlank(url)) {
            throw new QxException(ExceptionEnums.CREATE_PAY_URL_ERROR);
        }
    ...
    public Order queryById(Long orderId) {
        ...
        if (order == null) {
            throw new QxException(ExceptionEnums.ORDER_NOT_FOUND);
        }
        OrderDetail orderDetail = new OrderDetail();
        orderDetail.setOrderId(orderId);
